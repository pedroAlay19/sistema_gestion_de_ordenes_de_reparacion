...
export const useWebSocket = (options: UseWebSocketOptions = {}) => {
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      ws.onerror = (error) => {
        console.error("⚠️ WebSocket connection error. Make sure Go server is running on port 8081");
        isConnectingRef.current = false;
      };
      ...
      ws.onclose = (event) => {
        ...
        if (event.code !== 1000 && reconnectAttemptsRef.current < maxReconnectAttempts) {
          ...
          console.log("⚠️ WebSocket reconnection failed. Dashboard will work without real-time updates.");
        }
      };
      ...
      console.log("⚠️ Dashboard will work without real-time updates");
      isConnectingRef.current = false;
    }
  }, [onDashboardUpdate]);
